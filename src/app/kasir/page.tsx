...
export default function KasirPage() {
  ...
  // --- STATE POP-UP NOTIFIKASI & KONFIRMASI CUSTOM ---
  ...
  // --- 1. CEK USER LOGIN & AMBIL PENGATURAN ADMIN ---
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        try {
          ...
          if (userDoc.exists()) {
            setCashierName(userDoc.data().nama || user.email);
          } else {
            setCashierName(user.email || "Kasir");
          }
        } catch (error) {
    ...
    // Ambil Pengaturan PPN dan Diskon dari Pengaturan Admin di Firestore
  ...
  // --- 3. BUAT ID TRANSAKSI OTOMATIS ---
  useEffect(() => {
    if (cart.length > 0 && !currentTrxId) {
  ...
  const confirmAddToCart = () => {
    ...
    setCart((prevCart) => {
      ...
      if (existing) {
        return prevCart.map((item) => item.id === selectedProduct.id ? { ...item, quantity: item.quantity + tempQty } : item);
      }
  ...
  // --- KALKULASI OTOMATIS BERDASARKAN ATURAN ADMIN ---
  const subTotal = cart.reduce((acc, item) => acc + item.price * item.quantity, 0);
  
  // Diskon langsung memotong subtotal berdasarkan pengaturan admin
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setModalMessage("Gagal memproses transaksi.");
    } finally {
      ...
      try {
        if (lastTransaction?.id) {
          ...
          await addDoc(collection(db, "activity_logs"), {
            ...
            details: `Membatalkan Transaksi ID: ${lastTransaction.transactionId}`,
            timestamp: serverTimestamp()
          });
        }
      ...
      <div className="flex-1 flex flex-col no-print">
        <header className="bg-slate-900 shadow-md px-6 py-4 flex justify-between items-center z-10 text-white">
          <div className="flex items-center gap-4">
            ...
            <span className={`hidden sm:inline-flex px-3 py-1 rounded-full text-xs font-bold ${isOnline ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 bg-slate-900/90 z-[300] flex items-center justify-center backdrop-blur-sm no-print">
          <div className="bg-white p-8 rounded-3xl w-full max-w-sm shadow-2xl text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
            ...
            <h2 className="text-2xl font-extrabold text-zinc-900 mb-1">Transaksi Berhasil!</h2>
